...
?>
<div class='rapport'>
    <h4 class='text-center border-top border-bottom'>Participation du <?= utf8_encode(strftime('%A %e %B %Y',strtotime($part!=NULL?$part->date:$cren->date)))?> - <?=$part!=NULL?$part->heure:$cren->heure?> heures - <?= $lngNom ?></h4>
    ...
    <select name='langue' class='form-control <?= (form_error('langue') !== "") ? 'error' : '' ?>'>
        ...
        <?php foreach ($langs as $lang) { ?>
            <option value='<?= $lang->id ?>'  <?= ($idLang == $lang->id && $populate) ? 'selected' : '' ?>><?= $lang->intitule ?></option>
        <?php } ?>
    </select>

    <!--Nb personnes vues-->
    <input min="0" type="number" name="nbRenc" value="<?php echo $populate ? $renc->nb : ''; ?>" size="50" class='form-control <?= (form_error('nbRenc') !== "") ? 'error' : '' ?>' placeholder="Nombre de contacts"/>

           <?php
           /* Affichage du nombre de lignes publications requis (0 par défaut, le nombre de ligne déjà affichée si rechargement) */
           for ($cpt = 1; $cpt <= 10; $cpt++) {
               ?>
                <div id="publication<?= $cpt ?>" class="row">
                    <div class='col-sm-8'>
                        <select name='pub<?= $cpt ?>' id='pub<?= $cpt ?>'  class='form-control <?= (form_error('pub' . $cpt) !== "") ? 'error' : '' ?>'>
                            <option <?=isset($placs[$cpt-1])?'':'selected'?> value='-1'>Choisir une publication</option>
                            <?php foreach ($pubs as $pub) { ?>
                                <option value='<?= $pub->id ?>' <?= isset($placs[$cpt-1]) && $placs[$cpt-1]->idPub==$pub->id ? 'selected' : '' ?>><?= $pub->titre ?></option>
                            <?php } ?>
                        </select>
                    </div>
                    <div class='col-sm-4'>
                        <input min="1" type="number" name="qte<?= $cpt ?>" value="<?= isset($placs[$cpt-1])  ? $placs[$cpt-1]->qte : '' ?>" size="50"  class='form-control <?= (form_error('qte' . $cpt) !== "") ? 'error' : '' ?>' placeholder='Quantité'/>
                    </div>
                </div>
           ...
           for ($cpt = 1; $cpt <= 10; $cpt++) {
               ?>
                <div id="fait<?= $cpt ?>" class="row">
                    <div class='col-sm-12'>
                        <textarea name='fait<?= $cpt ?>' id='fait<?= $cpt ?>'  class='mb-4 form-control <?= (form_error('fait' . $cpt) !== "") ? 'error' : '' ?>'><?=isset($faits[$cpt-1])?$faits[$cpt-1]->fait:'Note les détails du fait ici...'?></textarea>
                    </div>
                </div>
    ...
    <div class='row'>
        ...
        <div class='col-sm-6'><button class='btn btn-lg btn-block btn-success' type="submit" id="fin"/>Sauvegarder</button></div>
        <div class='col-sm-6 hidden-phone'><a href="<?=site_url('rapport/consulter')."/$idRap"?>" class='btn btn-lg btn-block btn-danger' id="annuler"/>Annuler</a></div>
        <div class='col-sm-6 hidden-desktop hidden-tablet mobileLangAdd'><a href="<?=site_url('rapport/consulter')."/$idRap"?>" class='btn btn-lg btn-block btn-danger' id="annuler"/>Annuler</a></div>
    </div>
