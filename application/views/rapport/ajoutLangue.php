...
?>
<div class='rapport'>
    <h4 class='text-center border-top border-bottom'>Participation du <?= utf8_encode(strftime('%A %e %B %Y',strtotime($part!=NULL?$part->date:$cren->date)))?> - <?=$part!=NULL?$part->heure:$cren->heure?> heures </h4>
    ...
    <select name='langue' class='form-control <?= (form_error('langue') !== "") ? 'error' : '' ?>'>
        ...
        <?php foreach ($langs as $lang) { ?>
            <option value='<?= $lang->id ?>'  <?= (set_value('idLang') == $lang->id && $populate) ? 'selected' : '' ?>><?= $lang->intitule ?></option>
        <?php } ?>
    </select>
